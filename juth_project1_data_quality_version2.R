...
miss_ct_findings <- sqldf("select record_id, 
                          ...
                          hcc_ct_var_y, 
                          hcc_pvinvasion_y,
                          hcc_pvthrombosis_y from juth_data 
                          where redcap_event_name in ('enrollment_arm_1','enrollment_arm_2') and 
...
ecog_missing <- juth_data %>% filter(!(juth_data$visit_hcc_ecog %in% c(0,1,2,3,4 )) &((redcap_event_name == "visit_arm_1") | (redcap_event_name == "visit_arm_2"))) %>% select(record_id,redcap_event_name,visit_hcc_ecog)
