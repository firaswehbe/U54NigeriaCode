rm(list = ls())
##### This is synced into github as version2 script
...
enrform_hiv_y_missing <- sqldf("select record_id, redcap_event_name, enr_hiv_serology_y, enr_hcc_serology_y, 'Enrollment Form' as form
                               from juth_data where enr_hiv_serology_y is null and redcap_event_name = ('enrollment_arm_1' or  'enrollment_arm_2')")


enrform_hcc_y_missing <- sqldf("select record_id, redcap_event_name, enr_hiv_serology_y, enr_hcc_serology_y, 'Enrollment Form' as form
...
####HCC Diagnosis form
#############################have to edit this
hcc_y <- juth_data %>% filter(juth_data$enr_hcc_serology_y == 1)
...
enrform_hcc_ae_missing <- juth_data %>% filter(juth_data$enr_hcc_serology_y == 1 & (!hcc_artwashout_y %in% c(1,2,0) )) %>% select(record_id, redcap_event_name, hcc_artwashout_y )
#other way ####enrform_hcc_ae_missing <- juth_data %>% filter(juth_data$enr_hcc_serology_y == 1 & (hcc_artwashout_y %in% (NA) )) %>% select(record_id, redcap_event_name, hcc_artwashout_y )
...
enrform_hcc_bclc_missing <- juth_data %>% filter(juth_data$enr_hcc_serology_y == 1 & !(juth_data$hcc_bclc %in% c(0,1,2,3,4 )))  %>% select(record_id, redcap_event_name, hcc_bclc )
...
juth_data_hivinitial_enr <- juth_data %>% filter(juth_data$redcap_event_name == "enrollment_arm_1" & juth_data$enr_hiv_serology == 1 & (is.na(hiv_rna_pre_tx) | is.na(hiv_cd4_initial))) %>% select(record_id,redcap_event_name, hiv_rna_pre_tx,hiv_cd4_initial)
...
juth_data_hivinitial_enr2 <- juth_data %>% filter(juth_data$redcap_event_name == "enrollment_arm_2" & juth_data$enr_hiv_serology == 1 &  (is.na(hiv_rna_pre_tx) | is.na(hiv_cd4_initial))) %>% select(record_id, redcap_event_name, hiv_rna_pre_tx,hiv_cd4_initial)
...
hiv_y <- juth_data %>% filter(enr_hiv_serology_y == "1") %>% select ( record_id,redcap_event_name, enr_otherid)
hiv_us <- juth_data %>% filter(visit_hiv_us___0 == 0 & visit_hiv_us___1 == 0 & visit_hiv_us___2 == 0 & visit_hiv_us___3 == 0 & visit_hiv_us___4 == 0 & visit_hiv_us___5 == 0 & visit_hiv_us___6 == 0 & visit_hiv_us___99 == 0) %>% select(record_id,visit_hiv_us___0,visit_hiv_us___1,visit_hiv_us___2,visit_hiv_us___3,visit_hiv_us___4,visit_hiv_us___5,visit_hiv_us___6,visit_hiv_us___99)
...
visit_ht <- juth_data %>% filter(redcap_event_name == "visit_arm_1" | redcap_event_name == "visit_arm_2") %>% mutate(form = "Visit form") %>% select(record_id, visit_ex_height, redcap_event_name, form)
...
visit_wt <- juth_data %>% filter(redcap_event_name == "visit_arm_1" | redcap_event_name == "visit_arm_2") %>% mutate(form = "Visit form") %>% select(record_id, visit_ex_weight, redcap_event_name, form)
...
###hiv and hcc missing
visit_hivy <- juth_data %>% filter(redcap_event_name == "visit_arm_1" | redcap_event_name == "visit_arm_2") %>% mutate(form = "Visit form") %>% select(record_id, visit_hiv_y, redcap_event_name, form)
...
visit_hccy <- juth_data %>% filter(redcap_event_name == "visit_arm_1" | redcap_event_name == "visit_arm_2") %>% mutate(form = "Visit form") %>% select(record_id, visit_hcc_y, redcap_event_name, form) 
...
###ecog missing
#hcc_diagnosis_y <- juth_data %>% filter(enr_hcc_serology_y == "1" & (redcap_event_name == "enrollment_arm_1" | redcap_event_name == "enrollment_arm_2")) %>% mutate(form = "Visit form") %>% select(record_id, form)
ecog_missing <- juth_data %>% filter(!(juth_data$visit_hcc_ecog %in% c(0,1,2,3,4 )) &((redcap_event_name == "visit_arm_1") | (redcap_event_name == "visit_arm_2"))) %>% mutate(form = "Visit form")  %>% select(record_id,redcap_event_name,visit_hcc_ecog,form)
...
###fibroscan
fibroscan <-juth_data %>% filter(redcap_event_name == "visit_arm_1" | redcap_event_name == "visit_arm_2") %>% select (record_id, redcap_event_name, visit_inv_fibro)
...
hiv_diagnosis_y <- juth_data %>% filter(enr_hiv_serology_y == "1" & (redcap_event_name == "enrollment_arm_1" | redcap_event_name == "enrollment_arm_2")) %>% mutate(hiv_diagnosis = "Yes") %>% select(record_id, enr_otherid, hiv_diagnosis)
hiv_treatment_y <- juth_data %>% filter(visit_hiv_rx_y == "1")  %>% select (record_id,redcap_event_name,visit_hiv_rx_y, visit_hiv_rx_sd,visit_hiv_rx_reg)
...
hbv_treatment_y <- juth_data %>% filter(visit_hbv_rx_y == "1")  %>% select (record_id,redcap_event_name,visit_hbv_rx_y, visit_hbv_rx_sd,visit_hbv_rx_reg)
hbv_record <- juth_data %>% filter(redcap_event_name == "enrollment_arm_1" | redcap_event_name == "enrollment_arm_2") %>% select(record_id, enr_otherid)
...
visit_j <- juth_data %>% filter(redcap_event_name == "visit_arm_1" | redcap_event_name == "visit_arm_2") %>% mutate(form = "Visit form") %>% select(record_id, visit_sx_jaun_y, redcap_event_name, form) %>% filter(is.na(visit_sx_jaun_y))
...
visit_enc <- juth_data %>% filter(redcap_event_name == "visit_arm_1" | redcap_event_name == "visit_arm_2") %>% mutate(form = "Visit form") %>% select(record_id, visit_ex_liver_enc_y, redcap_event_name, form)
...
hiv_diagnosis_n <- juth_data %>% filter((enr_hiv_serology_y == "1") & (redcap_event_name == "enrollment_arm_1" | redcap_event_name == "enrollment_arm_2")) %>% select(record_id, enr_otherid, enr_hiv_serology_y)
lab_result <- juth_data %>% filter(redcap_event_name == "visit_arm_1" | redcap_event_name == "visit_arm_2") %>% select(record_id,redcap_event_name, visit_lab_cd4cnt_res, visit_lab_hivrna_res)
...
oc_missing1 <- juth_data %>% filter(final_death_y == "1") %>% mutate(death = "yes") %>% select (record_id, death, final_death_d, final_death_causes)
oc_missing2 <- juth_data %>% filter(redcap_event_name == "enrollment_arm_1" | redcap_event_name == "enrollment_arm_2") %>% select (record_id,redcap_event_name, enr_otherid)
...
outcome_missing <- sqldf("select record_id,redcap_event_name, enr_otherid, final_death_y, final_death_d, final_death_causes,'Outcome form' as form
                         from juth_data
                         where final_death_y = '1' and (final_death_d is null or final_death_causes is null)")
outcome_var_missing<- data.frame(outcome_missing, missing =apply(outcome_missing,1,sumna)) %>% select(record_id,redcap_event_name, form, missing)
...
u54_id <- mydata %>% select(record_id, enr_otherid, enr_hiv_serology_y, enr_hcc_serology_y)
...
agg_missing <- sqldf("select i.record_id as Record_id, u.enr_otherid as Other_ID, 
                     ...
                     when enr_hiv_serology_y = '1' then 'HIV+'
                      ...
                      case
                     when enr_hcc_serology_y = '1' then 'HCC+'
                     else 'HCC-'
                     end HCC,
                      redcap_event_name as Event, form as Form, fields as Fields, missing as Missing
                     from item_missing i
...
visit_bmi <- juth_data %>% filter(redcap_event_name == "visit_arm_1" | redcap_event_name == "visit_arm_2") %>% mutate(form = "Visit form") %>% select(record_id, visit_sx_bmi, redcap_event_name, form)
visit_bmi <- sqldf("select i.record_id as Record_id, u.enr_otherid as Other_ID, visit_sx_bmi, redcap_event_name, form as Form
                   from visit_bmi i
                   left join u54_id u on i.record_id =u.record_id
                   group by i.record_id, form
                   order by i.record_id")
visit_bmi <- visit_bmi %>% filter(visit_sx_bmi >50 | visit_sx_bmi <10) %>% select(Record_id,Other_ID, visit_sx_bmi, redcap_event_name, Form)
names(visit_bmi) [names(visit_bmi) == "visit_sx_bmi"] = "BMI"
write_csv(visit_bmi, '~/Desktop/juth_invalid_values.csv')
#create markdown report
rmarkdown::render('juth_dq_version2.Rmd', output_dir = 'output/juth', run_pandoc = TRUE)
rmarkdown::render('juth_dq_invalid.Rmd', output_dir = 'output/juth', run_pandoc = TRUE)

#rmarkdown::render("R:\PrevMed\Projects\QDSC\Projects\07 Nigeria U54\juth_dq_version2.Rmd", output_dir = 'R:\PrevMed\Projects\QDSC\Projects\07 Nigeria U54\project1_version2_dataquality_reports', run_pandoc = TRUE)
new <- paste("output/juth/juth_dq_",Sys.Date() ,".html", sep = "")
new2 <- paste("output/juth/juth_dq_invalid_",Sys.Date() ,".html", sep = "")
#print(new)
file.rename('output/juth/juth_dq_version2.html', new)
