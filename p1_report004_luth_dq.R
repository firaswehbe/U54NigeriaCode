...
####HCC Diagnosis form
#############################have to edit this
hcc_y <- luth_data %>% filter(luth_data$enr_hcc_serology_y == 1)
...
luth_data_hivinitial_enr <-  luth_data %>% filter( luth_data$redcap_event_name == "enrollment_arm_1" & luth_data$enr_hiv_serology == 1 &  (is.na(hiv_rna_pre_tx))) %>% 
...
luth_data_hivinitial_enr2 <- luth_data %>% filter(luth_data$redcap_event_name == "enrollment_arm_2" & luth_data$enr_hiv_serology == 1 & (is.na(hiv_rna_pre_tx))) %>% 
...
ecog_missing <- luth_data %>% filter(!(luth_data$visit_hcc_ecog %in% c(0,1,2,3,4 )) &((redcap_event_name == "visit_arm_1") | (redcap_event_name == "visit_arm_2"))) %>% mutate(form = "Visit form")  %>% select(record_id,redcap_event_name,visit_hcc_ecog,form)
...
### Hard coding 'out of study' subjects
agg_missing <- sqldf("select i.record_id as Record_id, u.enr_otherid as Other_ID, 
                     ...
                     where u.enr_otherid  not in ('LHCC005B','LHCC009','LHCC017','LHCC018',
                                   'LHCC021','LHCC149','LHCC164','LHCC168',
                                   'LHCC177','LHCC181','LHCC186','LHCC187',
                                   'LHCC188','LHCC305')
...
visit_bmi <- luth_data %>% filter(redcap_event_name == "visit_arm_1" | redcap_event_name == "visit_arm_2") %>% mutate(form = "Visit form") %>% select(record_id, visit_sx_bmi, redcap_event_name, form)
...
#rmarkdown::render("R:\PrevMed\Projects\QDSC\Projects\07 Nigeria U54\luth_dq_version2.Rmd", output_dir = 'R:\PrevMed\Projects\QDSC\Projects\07 Nigeria U54\project1_version2_dataquality_reports', run_pandoc = TRUE)
new <- paste("output/luth/luth_dq_",Sys.Date() ,".html", sep = "")
new2 <- paste("output/luth/luth_dq_invalid_",Sys.Date() ,".html")
#print(new)
file.rename('output/luth/luth_dq_version2.html', new)
